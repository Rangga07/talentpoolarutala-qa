...
WebUI.navigateToUrl('https://tokopedia.com/page_transaction')

def frontEndElement = findTestObject('name=deliveryServiceName')

def expectedText = 'TIKI'
...
def query = 'SELECT DELIVERY_SERVICE FROM transaction WHERE TRX_ID = \'01023A9AC\''

SqlResultSet result = CustomKeywords.'com.kms.katalon.core.database.custom.CusDatabaseKeywords.executeQuery'(db, query)

def layananPengirimanDatabase = hasilQuery.getRow(1).getString('DELIVERY_SERVICE')
