...
def frontEndElement = findTestObject('Page_Transaction/name=dateOrder')

def expectedText = '21/08/2016'
...
def query = 'SELECT DATE_ORDER FROM transaction WHERE TRX_ID = \'01023A9AC\''

SqlResultSet result = CustomKeywords.'com.kms.katalon.core.database.custom.CusDatabaseKeywords.executeQuery'(db, query)

def layananPengirimanDatabase = hasilQuery.getRow(1).getString('DATE_ORDER')
